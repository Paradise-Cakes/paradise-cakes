...
resource "aws_acm_certificate" "paradise_cakes_dev_api" {
  count             = var.environment == "prod" ? 0 : 1
...
resource "aws_acm_certificate_validation" "paradise_cakes_dev_api" {
  count           = var.environment == "prod" ? 0 : 1
  certificate_arn = aws_acm_certificate.paradise_cakes_dev_api[0].arn
}
